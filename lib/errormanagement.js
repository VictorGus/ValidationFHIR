...
class ValidationErrorManager {
    ...
    constructor(arrayOfErrors, schema, resource) {
        this.resourceAsObject = resource.resourceAsObject;
        this.schemaAsObject = schema.schemaAsObject;
        if(Array.isArray(arrayOfErrors)) {
            this.arrayOfErrors = this.filterErrors(arrayOfErrors, schema.schemaAsArrayOfKeys);
        }
        this.schema = schema.schemaAsArrayOfKeys;
        this.resource = resource.resourceAsArrayOfKeys;
    ...
    filterErrors(arrayOfErrors, schemaKeys) {
        ...
        var bundleKeys = [];
        if(this.resourceAsObject.resourceType == "Bundle") {
            bundleKeys = Object.keys(this.schemaAsObject.definitions[`Bundle`].properties);
            filteredArray = filteredArray.filter(element => !bundleKeys.includes(element.params.additionalProperty) && element); 
            for(let i = 0; i < this.resourceAsObject.entry.length; i++) {
                bundleKeys = Object.keys(this.schemaAsObject.definitions[`${this.resourceAsObject.entry[i].resource.resourceType}`].properties);
                filteredArray = filteredArray.filter(element => !bundleKeys.includes(element.params.additionalProperty) && element);
            }
            return this.getUniqueErrors(filteredArray);
        }
    ...
    isResourceTypeValid (schema, resource) {
